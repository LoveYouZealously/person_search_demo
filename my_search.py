...
def detect(cfg,
    ...
    if opt.webcam:
        save_images = False
        dataloader = LoadWebcam(pipe=1, img_size=img_size, half=opt.half)
    else:
    ...
    for i, (path, img, im0, vid_cap) in enumerate(dataloader):
        ...
        if len(os.listdir('query'))<5:
            ...
            if query_time_now-query_time_last>10:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dist_thres', type=float, default=1000.0, help='行人图片距离阈值，小于这个距离，就认为是该行人')
